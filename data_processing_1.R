...
#upload raw excel file, tab 1 containing summary data 
raw <- read.xlsx(xlsxFile = "with cetnred phosphosites Copy of ZR75-1FR tumor phosphoproteomics (Dec 2017)(9578) (1).xlsx", sheet = 1)
## data processing ----
...
raw_filter <- raw[,!colnames(raw) %in% vars_eliminate] # actually removing called columns above
...
sum(is.na(ctrl_means)) # checks to make sure everything got a number (shoud be 0 if worked)
...
## calculate fold change log2(B)-log2(A) ----
...
## save file with clean data and treatment mean stats ----

write.csv(raw_filter_complete, file="complete_stats.csv")
# ***** pull up file
...
#heat map
heat_palette <- colorRampPalette(c('blue', 'White', 'Red'))(n = 200)


heat_map <- raw_filter_complete[, 5:13]
heat_map_matrix <- data.matrix(heat_map)


pheatmap(heat_map_matrix)


#remove incomplete data from treatment means
heat_map_matrix_complete <- heat_map_matrix[complete.cases(heat_map_matrix[ , 1:9]),] # keep complete data
colnames(heat_map_matrix)
